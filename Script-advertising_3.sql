-- Rating of campaigns by ROMI by each month: 
...
rank() over (partition by ad_month order by ROMI desc) as ROMI_ratio,  
...
from CTE9
order by ad_month, ROMI_ratio asc;

-- Rating of campaign's CTR by each month: 
with CTE5 as(select ad_date, --TABLE WITH ALL data needed FOR google AND facebook campaigns
campaign_name, 
adset_name, 
spend, 
...
group by ad_month, utm_campaign)
select *,--comparison CPM, ROMI, CTR and CPC metrics with results in previos month  
rank() over (partition by ad_month order by CTR desc) as CTR_ratio,
case when prev_CPM = 0 then 0 
else CPM/prev_CPM*100::numeric end as CPM_diff,
case when prev_ROMI = 0 then 0 
else ROMI/prev_ROMI*100::numeric end as ROMI_diff,
...
from CTE9
order by ad_month, CTR_ratio asc;

-- Rating of campaigns by CPC by each month: 
with CTE5 as(select ad_date, --TABLE WITH ALL data needed FOR google AND facebook campaigns
campaign_name, 
adset_name, 
spend, 
...
group by ad_month, utm_campaign)
select *, --comparison CPM, ROMI, CTR and CPC metrics with results in previos month
rank() over (partition by ad_month order by CPC desc) as CPC_ratio,   
case when prev_CPM = 0 then 0 
else CPM/prev_CPM*100::numeric end as CPM_diff,
case when prev_ROMI = 0 then 0 
else ROMI/prev_ROMI*100::numeric end as ROMI_diff,
...
from CTE9
order by ad_month, CPC_ratio asc;

-- Rating of campaigns by CPM by each month: 
with CTE5 as(select ad_date, --TABLE WITH ALL data needed FOR google AND facebook campaigns
campaign_name, 
adset_name, 
spend, 
...
group by ad_month, utm_campaign)
select *, --comparison CPM, ROMI, CTR and CPC metrics with results in previos month
rank() over (partition by ad_month order by CPM desc) as CPM_ratio,   
case when prev_CPM = 0 then 0 
else CPM/prev_CPM*100::numeric end as CPM_diff,
case when prev_ROMI = 0 then 0 
else ROMI/prev_ROMI*100::numeric end as ROMI_diff,
...
from CTE9
order by ad_month, CPM_ratio asc;